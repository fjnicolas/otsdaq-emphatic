//
//  emphatic-artdaq/Generators/CAENV1720Readout_generator.cc
//  Copied from 1730 code - not confirmed to work yet
//

#define TRACE_NAME "CAENV1720Readout"
#include "artdaq/DAQdata/Globals.hh"

#include "CAENV1720Readout.hh"
...
// constructor of the CAENV1720Readout. It wants the param set
// which means the fhicl paramters in CAENV1720Readout.hh

emphaticdaq::CAENV1720Readout::CAENV1720Readout(fhicl::ParameterSet const& ps)
    : CommandableFragmentGenerator(ps), fCAEN(ps), fAcqMode(CAEN_DGTZ_SW_CONTROLLED)
{
	uint32_t data;

	TLOG_ARB(TCONFIG, TRACE_NAME) << "CAENV1720Readout()" << TLOG_ENDL;
//	TLOG(TCONFIG) << fCAEN;
	...
	// TML: new loop over all 4 boards
	for(unsigned int iboard = 0; iboard < fNBoards; iboard++)
	{
		uint32_t fBoardID = iboard;
		fHandle.push_back(-1);

		TLOG(TCONFIG) << ": Using BoardID=" << iboard << " with NChannels=" << fNChannels;

		TLOG(TDEBUG) << " Calling CAEN_DGTZ_OpenDigitizer(" << CAEN_DGTZ_OpticalLink << ", " << fCAEN.link << ", "
		             << iboard << ", " << 0 << " , " << fHandle[fBoardID] << ")";  // AA: debug
		retcode = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_OpticalLink, fCAEN.link, iboard, 0, &fHandle[fBoardID]);
		...
		if(retcode != CAEN_DGTZ_Success)
		{
			...
			fHandle[fBoardID] = -1;
			fOK               = false;
			TLOG(TLVL_ERROR) << ": Fatal error configuring CAEN board at " << fCAEN.link << ", " << iboard;
	...
	share::ThreadFunctor functor = std::bind(&CAENV1720Readout::GetData, this);
	auto                 worker_functor =
...
void emphaticdaq::CAENV1720Readout::configureInterrupts(unsigned int iboard)
{
	uint32_t fBoardID = iboard;
	...
	if(state != stateOut)
	{
		TLOG_WARNING("CAENV1720Readout") << "Interrupt State was not setup properly, state write/read="
		                                 << int32_t{state} << "/" << int32_t{stateOut};
	}
	if(eventNumber != eventNumberOut)
	{
		TLOG_WARNING("CAENV1720Readout") << "Interrupt State was not setup properly, eventNumber write/read="
		                                 << uint32_t{eventNumber} << "/" << uint32_t{eventNumberOut};
	}
	if(statusId != statusIdOut)
	{
		TLOG_WARNING("CAENV1720Readout") << "Interrupt StatusID was not setup properly, eventNumber write/read="
	...
	// {
	//   TLOG_WARNING("CAENV1720Readout") << "Interrupt State was not setup properly,
	...
	retcode = emphaticdaq::CAENV1720Readout::WriteRegisterBitmask(fHandle[fBoardID], addr, data, bitmask);
...
void emphaticdaq::CAENV1720Readout::loadConfiguration(fhicl::ParameterSet const& ps)
{
	// initialize the fhicl parameters (see CAENV1720Readout.hh)
	// the obj ps has a member method that gets he private members
	// fVerbosity, etc.. are priv memb in CAENV1720Readout.hh
	//
...
void emphaticdaq::CAENV1720Readout::Configure(unsigned int iboard)
{
	uint32_t fBoardID = iboard;
...
void CAENV1720Readout::ReadChannelBusyStatus(int handle, uint32_t ch, uint32_t& status)
{
	status               = 0xdeadbeef;
...
CAEN_DGTZ_ErrorCode CAENV1720Readout::ReadSPIRegister(int handle, uint32_t ch, uint32_t address, uint8_t* value)
{
	uint32_t            SPIBusy           = 1;
...
CAEN_DGTZ_ErrorCode CAENV1720Readout::WriteSPIRegister(int handle, uint32_t ch, uint32_t address, uint8_t value)
{
	uint32_t            SPIBusy = 1;
	CAEN_DGTZ_ErrorCode retcod  = CAEN_DGTZ_Success;
...
void emphaticdaq::CAENV1720Readout::ConfigureDaisyChain(uint32_t iboard)
{
	uint32_t            fBoardID = iboard;
	CAEN_DGTZ_ErrorCode retcode;
	uint32_t            readback;
...
void emphaticdaq::CAENV1720Readout::ConfigureSelfTriggerMode(uint32_t iboard)
{
	uint32_t            fBoardID = iboard;
	CAEN_DGTZ_ErrorCode retcod   = CAEN_DGTZ_Success;
...
void emphaticdaq::CAENV1720Readout::ConfigureRecordFormat(uint32_t iboard)
{
	uint32_t fBoardID = iboard;
...
emphaticdaq::CAENV1720Readout::~CAENV1720Readout()
{
	TLOG_ARB(TCONFIG, TRACE_NAME) << "~CAENV1720Readout()" << TLOG_ENDL;
	...
	TLOG_ARB(TCONFIG, TRACE_NAME) << "~CAENV1720Readout() done." << TLOG_ENDL;
}
...
//  unchanged
CAEN_DGTZ_ErrorCode emphaticdaq::CAENV1720Readout::WriteRegisterBitmask(int32_t  handle,
                                                                        uint32_t address,
                                                                        uint32_t data,
                                                                        uint32_t bitmask)
{
...
void emphaticdaq::CAENV1720Readout::ConfigureDataBuffer(uint32_t iboard)
{
	uint32_t fBoardID = iboard;
...
void emphaticdaq::CAENV1720Readout::ConfigureTrigger(uint32_t iboard)
{
	uint32_t fBoardID = iboard;
...
void emphaticdaq::CAENV1720Readout::ConfigureReadout(uint32_t iboard)
{
	uint32_t fBoardID = iboard;
...
void emphaticdaq::CAENV1720Readout::ConfigureAcquisition(unsigned int iboard)
{
	uint32_t fBoardID = iboard;
...
bool emphaticdaq::CAENV1720Readout::WaitForTrigger(uint32_t iboard)
{
	uint32_t fBoardID = iboard;
	...
	if(retcode != CAEN_DGTZ_Success)
	{
		TLOG_WARNING("CAENV1720Readout") << "Trying ReadRegister ACQUISITION_STATUS again." << TLOG_ENDL;
		retcode = CAEN_DGTZ_ReadRegister(fHandle[fBoardID], CAEN_DGTZ_ACQ_STATUS_ADD, &acqStatus);
	}
...
void emphaticdaq::CAENV1720Readout::start()
{
	if(fVerbosity > 0)
		TLOG_INFO("CAENV1720Readout") << "start()" << TLOG_ENDL;
...
void emphaticdaq::CAENV1720Readout::stop()
{
	if(fVerbosity > 0)
		TLOG_INFO("CAENV1720Readout") << "stop()" << TLOG_ENDL;
...
bool emphaticdaq::CAENV1720Readout::checkHWStatus_()
{
	for(unsigned int iboard = 0; iboard < fNBoards; iboard++)
	{
		uint32_t fBoardID = iboard;
...
bool emphaticdaq::CAENV1720Readout::GetData()
{
...
}  // CAENV1720Readout::GetData()

bool emphaticdaq::CAENV1720Readout::readWindowDataBlocksFromBoard(unsigned int iboard)
{
	uint32_t fBoardID = iboard;
...
bool emphaticdaq::CAENV1720Readout::readWindowDataBlocks()
{
	if(fail_GetNext)
	{
...
bool emphaticdaq::CAENV1720Readout::readSingleWindowDataBlock()
{
...
bool emphaticdaq::CAENV1720Readout::getNext_(artdaq::FragmentPtrs& fragments)
{
...
bool emphaticdaq::CAENV1720Readout::readSingleWindowFragments(artdaq::FragmentPtrs& fragments)
{
	...
	if(fPoolBuffer.activeBlockCount() == 0)
	{
		       ...
		       .count() > spill_timeout_ms_)
		{
			...
			endOfSubrunFrag->setSequenceID(last_sent_rwcounter + 1);

			endOfSubrunFrag->setTimestamp(++subrun_number_);
	...
	while(fPoolBuffer.activeBlockCount())
	{
		...
		if(!fPoolBuffer.read(range))
			break;

		const auto header = reinterpret_cast<CAENV1720EventHeader const*>(fragment_uptr->dataBeginBytes());
		...
		TLOG(TLVL_WARNING) << "boardid=" << header->boardID
...
bool emphaticdaq::CAENV1720Readout::readCombinedWindowFragments(artdaq::FragmentPtrs&)
{
	throw std::runtime_error("Implement me correctly!");
}

void emphaticdaq::CAENV1720Readout::CheckReadback(
...
DEFINE_ARTDAQ_COMMANDABLE_GENERATOR(emphaticdaq::CAENV1720Readout)
